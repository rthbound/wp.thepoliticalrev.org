	...
	<form method="post" action="<?php print esc_url(add_query_arg('settings-updated', 'true', $_SERVER['REQUEST_URI'])); ?>">
		...
		<p>
			<?php
				...
				echo sprintf(__('N.B. Generating two-factor codes depends upon your web-server and your TFA app/device agreeing upon the time. The current UTC time according to the server when this page loaded: %s', 'two-factor-authentication'), htmlspecialchars($date_now));
			?>
		</p>
		<p>
		<?php
